<div x-data="{
    ...
    <button value="copy" onclick="copyToClipboard('{{$item}}','{{$id}}')"
        class="block p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100">
        <div id="copia{{$id}}">
            <span>{{$item}}</span>
            <svg class="w-4 h-4 ml-1.5 stroke-current" xmlns="http://www.w3.org/2000/svg" fill="none"
        ...
        <div id="copiado{{$id}}" hidden>
            ...
            <svg class="w-4 h-4 ml-1.5 text-green-500 stroke-current" xmlns="http://www.w3.org/2000/svg" fill="none"
